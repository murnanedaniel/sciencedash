...
export async function buildBrainChatContext(): Promise<string> {
  ...
  parts.push(
    ...
    "  `create_hypothesis(projectId, title, ...)`, `create_check_in(projectId, body, ...)`,",
    "  `add_note(projectId, title, ...)`, `record_decision(...)`, `post_message(...)`.",
    "- Update: `set_project_blocker(projectId, blockers?)`,",
    "  `update_project_fields(projectId, ...)`, `update_hypothesis_status(id, ...)`,",
    "  `attach_project_to_programme(projectId, programmeId?)`,",
