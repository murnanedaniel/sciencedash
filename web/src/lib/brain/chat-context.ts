...
export async function buildBrainChatContext(): Promise<string> {
  ...
  parts.push(
    ...
    "through a Claude REPL on their local machine. The `sciencedash` skill is",
    "installed — use it for state queries and writes whenever the question is",
    "about projects, hypotheses, runs, decisions, blockers, or agent traffic.",
    "Every tool below is reached as `sd.py call <name> '<json-args>'`.",
    "Do NOT infer state from cwd / git history — the cwd is just a workspace",
    "directory; the live state is in the dashboard DB, reached via the skill.",
    "",
    "## Voice contract",
    "",
    ...
    "",
    "## Tool surface (via the `sciencedash` skill)",
    "",
    "Each tool below is invoked as `sd.py call <name> '<json-args>'` — the",
    "`name(args)` notation here is just shorthand for which tool + which args.",
    ...
    "",
    "Skills are recipes — they tell you which tools to call and in what order.",
